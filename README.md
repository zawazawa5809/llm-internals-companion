...
uv run python token_loop.py --demo-tokenize --model mlx-community/Llama-3.2-1B-Instruct-4bit

# 自己回帰ループを1トークンずつ + 各ステップの top-k を表示
uv run python token_loop.py --prompt "The capital of France is" --max-tokens 32

# 累積 wall-time の線形を実測 → results.jsonl（記事の chart-01 / tok/s の元データ）
uv run python token_loop.py --bench --max-tokens 128 --reps 3 --out results.jsonl
...
uv run python attention_from_scratch.py --demo

# 系列長 T スイープ: 射影(線形)と attention コア(二乗)の wall-time を分けて計測
uv run python attention_from_scratch.py --bench --reps 7 --out results.jsonl

# MLX 不要のロジック検証（softmax 行和=1・causal リーク無し）
uv run python attention_from_scratch.py --selftest
```
...
uv run python kv_cache_demo.py --probe --around 256 --span 6

# context 1K-32K で KV バイト数を実測（理論式・論理長・確保容量の三点一致）
uv run python kv_cache_demo.py --kv --contexts 1024,2048,4096,8192,16384,32768 --out results_kv.jsonl

# context 1K-32K で decode tok/s を実測（memory-bound の実測）
uv run python kv_cache_demo.py --decode --contexts 1024,2048,4096,8192,16384,32768 --out results_decode.jsonl

# mlx 不要のロジック検証（理論式・padding 計算）
uv run python kv_cache_demo.py --selftest
```
...
uv run python self_consistency.py --sample --model mlx-community/Qwen2.5-1.5B-Instruct-4bit \
  --m 20 --temperature 0.7 --top-p 0.95 --out results_samples.jsonl

# 保存済みサンプルから N∈{1,3,5,10,20} の maj@N 正答率を bootstrap CI 付きで算出（再生成なし）
uv run python self_consistency.py --analyze --in results_samples.jsonl --out results_analysis.jsonl

# mlx 不要のロジック検証（answer抽出・多数決・bootstrap CI）
uv run python self_consistency.py --selftest
```
...
uv run python tiny_gpt.py --train --out results.jsonl

# 学習パラメータを変える場合の例（縮小config・短時間で様子を見る）
uv run python tiny_gpt.py --train --n-layer 4 --n-head 4 --n-embd 128 --block-size 64 \
...
uv run python tiny_gpt.py --selftest
```
...
### 記事の数値（Apple M5 Pro 48GB / MPS / torch 2.12.1 / fp32 / 標準config, 実測パラメータ数10,795,776）

- train/val loss は5,000 iterationを通じて実測（21チェックポイント）。train lossは3.66→0.80まで単調に下がり続けるが、val lossはiteration 2,250で最小値1.4791（PPL 4.39, BPC 2.13）をつけたあと反転し、最終iteration(4,999)では1.5965（PPL 4.94, BPC 2.30）まで悪化する（過学習の実例）
- 参考値: nanoGPT公式ベンチマーク（A100 GPU実測・他者の数値のため比較の文脈情報としてのみ扱う）は同一configで validation loss 1.4697。自分のMPS実測の最小値1.4791とは差0.6%程度
- 所要時間: 約41分（2,445秒）。この回はセッション内で他プロセスと並行実行したため iteration 3,500-4,000 区間だけ他区間の3-8倍の時間がかかっている（純粋な学習時間の見積もりは27分程度）。実測値をそのまま報告する
- ピークメモリ: 約1.27GiB（1,360,550,400バイト、`current_allocated_memory()` を毎iteration backward+optimizer.step直後にサンプリングした近似値）
- 実装修正（`/code-review` high effortで発見・修正）: `--dtype` が forward に反映されない不整合、学習/評価バッチ間でのRNG共有（`--eval-iters`を変えると学習軌跡がずれる）、`torch.mps` 側RNG（dropout）が未シード、ピークメモリのサンプリングタイミングが評価チェックポイント後（backward直後の方がより正確）— 全て修正し、修正前後でval loss軌跡が実質的に同一であることを確認した上で最終数値を再計測
