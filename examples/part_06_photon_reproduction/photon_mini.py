...
decoder というチャンク階層構造を追加した photon_mini.py を同じデータ・同じ学習ループの上に構築する。
  ...
  - **Top-down decoder**:
    ...
    (4) ContextDecoder — 各 chunk を [R 本の prefix; chunk_size 個のtoken位置] という長さ R+C の窓に
        causal mask をかけて並列デコードする、chunk 間で重みを共有する局所自己回帰 Transformer。
        chunk g の decode は chunk (g-1) の encoder 状態 (= 1 つ前の coarse state) を条件とする
        (論文 p.3 の定義通り、chunk 自身の未来情報が prefix に漏れない設計)。
  ...
  - **KV 相当のメモリ削減の測る場所**: 学習 (teacher forcing) では毎回全系列を並列 forward するため
    ...
    *生成 (decode)* 時の話であるため、本ファイルは vanilla 用・photon_mini 用それぞれに
    `generate()` を実装し、生成中に実際に保持しているキャッシュテンソルのバイト数を introspect する
    (Part 3: kv_cache_demo.py と同じ実測方針)。vanilla は O(T)、photon_mini (HierGen) は
    O(T/chunk_size) で cache が増えることを、理論式と実測の一致で確認する。
"""
...
class PhotonMini(nn.Module):
    ...
    def generate_hiergen(self, idx: torch.Tensor, max_new_tokens: int):
        ...
        for _ in range(n_new_chunks):
            # coarse cache のバイト数を実測 (これがPHOTONのKV相当メモリ)
            cache_tensor = torch.cat(coarse_states, dim=1)
            cache_bytes = cache_tensor.element_size() * cache_tensor.nelement()
            mem_log.append({"n_coarse_states": len(coarse_states), "coarse_cache_bytes": cache_bytes})
